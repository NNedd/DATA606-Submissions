...
```
**The histogram produces a bell-shaped curve which indicated normal but it is slightly skewed as can be seen from the qq plot. **

###Typical Value

A typical value can be considered a measure of central tendency such as the mean or median.
```{r typical, eval =TRUE}
summary(samp)
```
If there are outliers in the sample the mean better measure of central tendency, however the mean is a more popular measure of central tendency. In this case the typical value is `r mean(samp)`.

2.  Would you expect another student's distribution to be identical to yours? 
Would you expect it to be similar? Why or why not?

**I would not expect another student's distibution to be identical since they may draw a different sample.  I would expect it to be similar however, since the sample is large enough to subscribe to the central limit theorem and be approximately normal.**
...
**For this confidence interval to be valid, the sample size must be at least 30 according to the Central Limit Theorem.**
...
**Yes the Confident interval captures the population mean**
...
**I would expect that at least 95% of the intervals would capture the population mean.  The 95% confidence still leaves a 5% chance that the interval would not capture the mean. *** 
...
-   Using the following function (which was downloaded with the data set), 
    ...
    ```
The proportion of intervals that do not include the population mean is:
```{r prop, eval= TRUE}
miss <- 0
for (i in 1:50){
...
prop
```
No the probability is not exactly equal to the confidence level since the Confidence level is merely a probability not intended to be an exact figure.
...
Since the "net is wider" so to speak, the confidence intervals captures more of the sample means and results in no misses.

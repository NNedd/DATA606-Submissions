...
2.  Describe the distribution of `score`. Is the distribution skewed? What does 
    ...
    ```{r dist-score, eval=TRUE}
    hist(evals$score)
    ```

The distribution of score is negatively skewed.  The students tend to rate scores at the high end of the scale.  This is what I expected to see because generally, at college level students pursue courses that they are interested in and would be more likely to evaluate the course higher.  In addition, students may be hesitant to give a low score when evaluating the course.
...
3.  Excluding `score`, select two other variables and describe their relationship 
    ...
    Scores selected age and bty_avg
...
**There are less points in the scatterplot compared to the number of observations in the database** 
...
```{r scatter-score-bty_avg2, eval = FALSE}
    plot(evals$score ~ jitter(evals$bty_avg))
  
```

The initial scatterplot hid values because of duplicates.
...
$$\hat{score} = 3.88034 + 0.06664 * (bty\_avg) $$  
**Yes, the variable bty_avg is both a statistically significant and practically significant predictor**
...
The bty_avg has a weak linear relationship with score.  In addition, from the graph, the condition of constant variability is not met.

**Second and Third Plots**

```{r second-plot, eval=TRUE}
hist(m_bty$residuals)
qqnorm(m_bty$residuals)
qqline(m_bty$residuals)

```
The distribution of the residuals is skewed.  This is confirmed by the qq plot.  The condition of nearly normal residuals is not met.
...
```
The residuals does not exhibit constant variablity.  In addition the distribution of residuals is skewed.
...
Yes, bty_avg is still a significant predictor.
Yes, the parameter estimate has also changed for bty_avg.
...
$$\hat{score} = 0.37474  + 0.07416 \times bty\_avg + 0.17239 $$
...
summary(m_bty_rank)
```
R appears to create two variables to deal with the three values of rank.  Maybe they assign the values 0, 1 and 2.
...
I would expect the proportion of students that filled out evaluations to have the highest p-value.
...
The highest p value ws actually cls_profssingle (marital status) with a p value of 0.778.  My supposition was wrong.



13. Interpret the coefficient associated with the ethnicity variable.

Professors who are not minorites score 0.12 points higher than those that are minorities.
...
```{r m_full2, eval = FALSE, tidy = FALSE}
...
The coefficients did not change much.  This is an indication of the unsuitabililty of the variable as a predictor but does not necessarily indicate that the variable was correlated with another in the model.  This would have resulted in a change of coefficient.
...
```{r m_full3, eval = FALSE, tidy = FALSE}
...
```{r m_full4, eval = FALSE, tidy = FALSE}
...
```{r m_full5, eval = FALSE, tidy = FALSE}
...
```{r m_full6, eval = FALSE, tidy = FALSE}
...
$$\hat{score} = 3.771922 + 0.167872 \times ethnicity + 0.207112 \times gender - 0.206178 \times language - 0.006046 \times age + 0.004656 \times cls\_perc\_eval + 0.505306 \times cls_credit + 0.051069 \times bty\_avg - 0.190579 \times pic\_color $$

16. Verify that the conditions for this model are reasonable using diagnostic 
    plots.
    
```{r verify-cond, eval=FALSE}
...
```
**The residuals are negatively skewed and does not fully satisfy the requirement for ccnstant variabililty and linearity.** 
...
Yes, since this would mean that the rows are not fully independent.
...
No.  Firstly, the conditions for acceptance were not fully realised.  Secondly, some cultures may very well see different results (may prefer minority and/or female for example.  Also may prefer non-english in countries that do not speak english)

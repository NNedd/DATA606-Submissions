...
** There are 1000 cases in the data set.  Each case represents the details about a birth in North Carolina.**
...
```{r habit-weight-boxplot, eval=TRUE}
...
smoking and non-smoking mothers are different.
  ...
  H0: Average weight of babies born to smoking and non-smoking mothers are not different
  
  H1: Average weight of babies born to smoking and non-smoking mothers are different
...
```{r inf-length-ci, eval=TRUE, tidy=FALSE}
...
```{r inf-length-ci-90, eval=TRUE, tidy=FALSE}
inference(y = nc$weeks, est = "mean", type = "ci", null = 0, 
...
```{r inf-young-weight-ht, eval=TRUE, tidy=FALSE}
...
```
There is no clear cutoff age for younger and mature mothers in the data.  Therefore the average of the limit values (highest younger mom and lowest mature mom) will be used as the cut off value: `r cut_off`
...
Variables:  fage and premie
Research Question: Is the average age of mothers who give birth to premature babies different than those that give birth to full term babies?

```{r inf-premie-fage-ht, eval=TRUE, tidy=FALSE}
inference(y = nc$fage, x = nc$premie, est = "mean", type = "ht", null = 0, 
          alternative = "twosided", method = "theoretical")
```
**Since the p value is > 0.05 we do not have enough evidence to reject the null hypothesis and conclude that there is no evidence to suggest that the average of mothers giving birth to premature babies is different than the average age of those mothers giving birth to full term babies.**

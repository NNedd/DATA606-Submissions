...
I would use a *scatterplot* to display the relationship between 'runs' and one of the other numerical variables.

```{r scatter-plot, eval=TRUE}
plot(mlb11$at_bats, mlb11$runs)
```

The data does not look perfectly linear but seems to be tending in a positive linear direction.
...
The relationship between the variables is linear in a positive direction.  The strength of the relationship is moderate.
...
```{r plotss-atbats-runs-min-squares, eval=TRUE}
plot_ss(x = mlb11$at_bats, y = mlb11$runs, showSquares = TRUE)
```
...
The slope of the line is positive therefore as homeruns increase, runs increase.
...
The team manager would predict a value close to 700.  The actual value is closer to 650 for a residual of approximately 50.
...
The relationship is not strongly linear only slightly
...
Again, it is not strongly normal.  There is slight tendency towards normality but it is not strong.
...
This condition does not appear to be met, there are some outliers.
* * *
...
Runs is a better predictor than at_bats.  We know this because the R$^2$ value is higher.
...
m4a <- lm(runs ~ hits, data = mlb11)
summary(m4a )


plot(mlb11$homeruns, mlb11$runs)
m4b <- lm(runs ~ homeruns, data = mlb11)
summary(m4b )

plot(mlb11$strikeouts, mlb11$runs)
m4c <- lm(runs ~ strikeouts, data = mlb11)
summary(m4c )

plot(mlb11$stolen_bases, mlb11$runs)
m4d <- lm(runs ~ stolen_bases, data = mlb11)
summary(m4d )

plot(mlb11$wins, mlb11$runs)
m4e <- lm(runs ~ wins, data = mlb11)
summary(m4e )


```   
  Of the five remaining variables, the best predictor is hits.
...
m5a <- lm(runs ~ new_onbase, data = mlb11)
summary(m5a )


plot(mlb11$new_obs, mlb11$runs)
m5b <- lm(runs ~ new_obs, data = mlb11)
summary(m5b )

plot(mlb11$new_slug, mlb11$runs)
m5c <- lm(runs ~ new_slug, data = mlb11)
summary(m5c )


```   

The predictions of runs usin the new variables are more accurate.  Overall, the new_obs variable seems to be the best predictor.  This makes the most sense since new_obs takes into consideration both slugging and the ability of the player to make it on base (a major way of making runs).  


-   Check the model diagnostics for the regression model with the variable you 
    decided was the best predictor for runs.

*Linearity*:  The data is not perfectly linear but tending towards linearity

```{r residuals2, eval=TRUE}
plot(m5b$residuals ~ mlb11$new_obs)
abline(h = 0, lty = 3)  # adds a horizontal dashed line at y = 0
```

*Nearly Normal*:  The data is slightly skewed but again, tending towards normality


```{r hist-res2, eval=TRUE}
hist(m5b$residuals)
```

*Constant variability*:  This seems to be met, there are no glaring outliers.

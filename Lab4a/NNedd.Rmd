...
```{r samp1_info, eval=TRUE}

hist(samp1)
```

**The distribution of the sample, while still slightly positively skewed, is closer to a normal distribution than that of the population.**
...
mean(samp2)
```
**The mean of the second sample is lower than the first but within the same vicinity.  I would assume that samples of size 100 and 1000 would be even closer to the population mean, the larger the sample the higher probability of a better the approximation to the population**

```{r larger_samps, eval=TRUE}
...
**Sample_means50 has 5000 elements.  The sampling distribution is normal with a centre of approximately 1500.  For 50,000 sample means I would expect the distribution to be more or less the same.**
...
```
**There are 100 elements in sample_means_small.  Each element represents the mean of a sample of 50 from area.**
...
**As the sample size gets larger the sample mean gets closer to the population mean and the spread decreases.**
...
```{r sample_price, eval = TRUE}
price_Samp <- sample(price, 50)
mean_smp_price <- mean(price_Samp)
mean_smp_price
```

My best estimate of the population mean is `r signif(mean_smp_price,3)`
...
```{r samp-price, eval=TRUE}
sample_means50 <- rep(NA, 5000)

for(i in 1:5000){
  samp_price <- sample(price, 50)
  sample_means50[i] <- mean(samp_price)
}

hist(sample_means50, breaks = 20)

```
**The distribution is bell-shaped (normal)
...
```{r samp-price150, eval=TRUE}
sample_means150 <- rep(NA, 5000)

for(i in 1:5000){
  samp_price <- sample(price, 150)
  sample_means150[i] <- mean(samp_price)
}
par(mfrow = c(2, 1))
hist(sample_means50, breaks = 20)
...
The shape of the sample_means150 distribution is also bell shaped, similarily to the sample_means50 distribution.

I would guess that the mean price is approximately $180,000
...
```{r spread, eval = TRUE}
sd50 <- sd(sample_means50)
sd150 <- sd(sample_means150)
if (sd50 < sd150){smaller <- "sample_means50"}
if (sd50 == sd150){smaller <- "equal spread"}
if (sd50 > sd150) {smaller <- "sample_means150"}
```
The sampling distribution with the smaller spread is `r smaller`.
I would prefer a distribution with a smaller spread since it would mean that the values are closer together and thus closer to the mean.

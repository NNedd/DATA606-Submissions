...
1.  In the first paragraph, several key findings are reported. Do these 
    ...
    **The percentages appear to be sample statistics since they reported the results of a poll.**
...
2.  The title of the report is "Global Index of Religiosity and Atheism". To
    ...
    **We must assume that the sampling method was random in order to generalise the reports finding to the global human population.**
...
3.  What does each row of Table 6 correspond to? What does each row of 
    ...
    * Each row in Table 6 corresponds to the poll results of a particular country.
    * Each row of the atheism data frame corresponds to a response by an individual noting the nationality of the person, their response and the year.
...
```{r prop-atheist, eval = TRUE}
table1 <- table(us12$response)
prop.table(table1)
```
...
* That the sample is independent
* That there are at least 10 successes and 10 failures.
...
The margin of error for this data according to the R output is approximately 4-6%    
...
```{r can-atheism, eval=TRUE}
can12 <- subset(atheism, nationality == "Canada" & year == "2012")
```

```{r prop-atheist2, eval = TRUE}
table2 <- table(can12$response)
table2
```

The sample was independent and there are at least 10 success and failures, so conditions exist for inference.

```{r can-atheism-ci, eval=TRUE, tidy = FALSE}
inference(can12$response, est = "proportion", type = "ci", method = "theoretical", 
          success = "atheist")
```
Country 2: Australia

```{r aus-atheism, eval=TRUE}
aus12 <- subset(atheism, nationality == "Australia" & year == "2012")
```

```{r prop-atheist3, eval = TRUE}
table3 <- table(aus12$response)
table3
```

The sample was independent and there are at least 10 success and failures, so conditions exist for inference.

```{r aus-atheism-ci, eval=TRUE, tidy = FALSE}
